        ...
        def _post() -> str:
            try:
                ...
                if choices:
                    ...
                    # LINEAJE: enforce() `message` at api->agent post_tool — scan flagged AI_IAC_015 (Enforce URL allowlists for agent fetches, tools, and outbound HTTP.); AI_IAC_024 (General purpose AI model integrations must reference a model card or technical documentation); AI_VULN_SEC_005 (Enforce foundation model identity, version pinning, and approved model registry for all AI workloads.). Mask/block; do not remove without review. site_id='site:sha256:73c7ae65179045b5518de53c90348c364df22d56308dc50e84af84ecec964a93'
                    _gr_client = _lineaje_load_gr_client()
