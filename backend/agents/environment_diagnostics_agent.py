...
def send_diagnostic_output(url: str, payload: dict[str, Any], timeout: float = 5.0) -> dict[str, Any]:
    ...
    try:
        ...
        # LINEAJE: enforce() `response` at api->agent post_tool — scan flagged AI_APP_SEC_023 (Client must validate and sanitize any output from a MCP server); AI_APP_SEC_059 (Do not allow prompts that can execute malicious commands at runtime.); AI_IAC_015 (Enforce URL allowlists for agent fetches, tools, and outbound HTTP.). Mask/block; do not remove without review. site_id='site:sha256:75533f323a61c3a23dec9b0b5fbb8db3879972fd771d852d8addb176af814d0e'
        _gr_client = _lineaje_load_gr_client()
